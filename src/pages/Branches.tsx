...
export default function Branches() {
  ...
  const addBranch = async (e: React.FormEvent) => {
    e.preventDefault();
    if (role?.role !== "manager") return toast.error("Only managers can add branches.");
    ...
    logActivity.mutate({
      ...
      description: `Branch "${trimmed}" created`,
    });
    setNewBranch("");
    refreshBranches();
  };
  ...
  const saveEdit = async (id: string) => {
    if (role?.role !== "manager") return toast.error("Only managers can edit branches.");
    const trimmed = editingName.trim();
    if (trimmed.length < 2) return toast.error("Branch name too short");
    if (trimmed.length > 60) return toast.error("Branch name too long");
    const old = branches.find((b) => b.id === id)?.name ?? "";
    setBusy(true);
    ...
    logActivity.mutate({
      ...
      description: `Branch renamed from "${old}" to "${trimmed}"`,
    });
    cancelEdit();
    refreshBranches();
  };
    ...
    <AppShell>
      ...
      <Card className="glass rounded-2xl p-6">
        <form onSubmit={addBranch} className="flex gap-2 mb-4">
          ...
          <Button
            type="submit"
            disabled={busy || role?.role !== "manager"}
        ...
        <ul className="space-y-2">
          {branches.map((b) => (
            <li
              key={b.id}
              className="flex items-center justify-between gap-2 p-3 rounded-lg bg-secondary/40"
            >
              {editingId === b.id ? (
                <>
                  <Input
          ...
          {!branches.length && (
            <p className="text-sm text-muted-foreground text-center py-6">No branches yet.</p>
          )}
        </ul>
      </Card>
    </AppShell>
