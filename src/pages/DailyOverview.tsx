...
export default function DailyOverview() {
  ...
  const productCode = useMemo(() => {
    if (selectedSubProduct !== ALL) return subProducts.find((s) => s.id === selectedSubProduct)?.code ?? "";
  ...
  const metrics = useMemo(() => {
    ...
    if (selectedSubProduct !== ALL) {
      ...
      const subIds = new Set(subProducts.filter((s) => s.product_id === selectedProduct).map((s) => s.id));
  ...
  const setSingleField = async (
    ...
    try {
      ...
      await upsert.mutateAsync(payload);
      toast.success(
        field === "target" ? "Morning Target updated" :
        field === "completed" ? "Evening Completed updated" :
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="rounded-xl border border-border/60 bg-secondary/40 p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={selectedProduct} onValueChange={setSelectedProduct} disabled={productDisabled}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={selectedSubProduct} onValueChange={setSelectedSubProduct} disabled={subDisabled}>
              <SelectTrigger className="h-9 text-sm">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input readOnly value={productCode} placeholder="—" className="h-9 text-sm bg-background/50" />
          </div>
          ...
          <div className="border-t border-border/60 pt-4 mt-1 space-y-3 text-sm">
            ...
            <div className="grid grid-cols-[1fr_7rem_auto] items-center gap-2">
              <Label className="text-muted-foreground" htmlFor="do-target">Morning Target</Label>
              <Input
                id="do-target"
                type="number"
                inputMode="numeric"
              ...
              <Button
                ...
                disabled={editingDisabled || settingField === "target"}
            ...
            <div className="grid grid-cols-[1fr_7rem_auto] items-center gap-2">
              <Label className="text-muted-foreground" htmlFor="do-completed">Evening Completed</Label>
              <Input
                id="do-completed"
                type="number"
                inputMode="numeric"
              ...
              <Button
                ...
                variant="secondary"
                className="h-9 px-3 rounded-md bg-success/15 text-success hover:bg-success/25 border border-success/30"
                disabled={editingDisabled || settingField === "completed"}
            ...
            <div className="grid grid-cols-[1fr_7rem_auto] items-center gap-2">
              <Label className="text-muted-foreground" htmlFor="do-manpower">Manpower Today</Label>
              <Input
                id="do-manpower"
                type="number"
                inputMode="numeric"
              ...
              <Button
                ...
                variant="secondary"
                className="h-9 px-3 rounded-md bg-success/15 text-success hover:bg-success/25 border border-success/30"
                disabled={editingDisabled || settingField === "manpower"}
