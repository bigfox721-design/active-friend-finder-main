import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import type { Product, ProductionEntry } from "@/lib/types";
import { todayISO } from "@/lib/format";
import { useBranch } from "./useBranch";
import { useCreateActivityLog } from "./useActivityLog";
import type { Tables } from "@/integrations/supabase/types";

export const useProducts = () => {
  const { branchId } = useBranch();
  return useQuery({
    queryKey: ["products", branchId],
    enabled: !!branchId,
    queryFn: async () => {
      let q = supabase.from("products").select("*").eq("active", true).order("name");
      if (branchId) q = q.eq("branch_id", branchId);
      const { data, error } = await q;
      if (error) throw error;
      return data;
    },
  });
};

export const useEntries = (opts?: { from?: string; to?: string }) => {
  const { branchId } = useBranch();
  return useQuery({
    queryKey: ["entries", branchId, opts?.from, opts?.to],
    enabled: !!branchId,
    queryFn: async () => {
      let q = supabase
        .from("production_entries")
        .select("*, product:products(*)")
        .order("entry_date", { ascending: false });
      if (branchId) q = q.eq("branch_id", branchId);
      if (opts?.from) q = q.gte("entry_date", opts.from);
      if (opts?.to) q = q.lte("entry_date", opts.to);
      const { data, error } = await q;
      if (error) throw error;
      return data as (ProductionEntry & { product: Product })[];
    },
  });
};

export const useTodayEntries = () => useEntries({ from: todayISO(), to: todayISO() });

export const useUpsertEntry = () => {
  const qc = useQueryClient();
  const { branchId } = useBranch();
  const logActivity = useCreateActivityLog();
  return useMutation({
    mutationFn: async (input: {
      product_id: string;
      entry_date: string;
      target_qty?: number;
      completed_qty?: number;
      notes?: string;
      manpower?: number;
      delay_reason?: string | null;
    }) => {
      const user = (await supabase.auth.getUser()).data.user;
      const userName = user?.email?.split("@")[0] ?? "Unknown";
      const { data: prodExists } = await supabase
        .from("products")
        .select("id")
        .eq("id", input.product_id)
        .maybeSingle();
      if (!prodExists) {
        const { data: sub } = await supabase
          .from("sub_products")
          .select("id, name, code, product_id, product:products(branch_id, unit)")
          .eq("id", input.product_id)
          .maybeSingle();
        if (sub) {
          await supabase.from("products").insert({
            id: sub.id,
            name: sub.name,
            code: sub.code ?? null,
            branch_id: (sub as any).product?.branch_id ?? branchId,
            unit: (sub as any).product?.unit ?? "pcs",
            active: false,
          } as any);
        }
      }

      const { data: existing } = await supabase
        .from("production_entries")
        .select("*")
        .eq("product_id", input.product_id)
        .eq("entry_date", input.entry_date)
        .maybeSingle();

      const isUpdate = !!existing;
      if (existing) {
        const patch: Record<string, number | string | null> = {};
        if (input.target_qty !== undefined) patch.target_qty = input.target_qty;
        if (input.completed_qty !== undefined) patch.completed_qty = input.completed_qty;
        if (input.notes !== undefined) patch.notes = input.notes;
        if (input.manpower !== undefined) patch.manpower = input.manpower;
        if (input.delay_reason !== undefined) patch.delay_reason = input.delay_reason;
        const { error } = await supabase
          .from("production_entries")
          .update(patch as Tables<"production_entries">)
          .eq("id", existing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("production_entries").insert({
          product_id: input.product_id,
          entry_date: input.entry_date,
          target_qty: input.target_qty ?? 0,
          completed_qty: input.completed_qty ?? 0,
          notes: input.notes ?? null,
          manpower: input.manpower ?? null,
          delay_reason: input.delay_reason ?? null,
          created_by: user?.id ?? null,
          branch_id: branchId,
        } as any);
        if (error) throw error;
      }

      const { data: prod } = await supabase
        .from("products")
        .select("name")
        .eq("id", input.product_id)
        .maybeSingle();
      const productName = prod?.name ?? input.product_id;
      logActivity.mutate({
        branch_id: branchId ?? undefined,
        product_id: input.product_id,
        action: isUpdate ? "entry_updated" : "entry_created",
        description: isUpdate
          ? `Production entry updated for ${productName}: ${Object.keys(input)
              .filter((k) => k !== "product_id" && k !== "entry_date")
              .join(", ")}`
          : `Production entry created for ${productName}`,
        user_name: userName,
      });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["entries"] });
      qc.invalidateQueries({ queryKey: ["inventory"] });
      qc.invalidateQueries({ queryKey: ["inventory_logs"] });
      qc.invalidateQueries({ queryKey: ["accessory_inventory"] });
    },
  });
};

export const useAddProduct = () => {
  const qc = useQueryClient();
  const { branchId } = useBranch();
  return useMutation({
    mutationFn: async (name: string) => {
      const { error } = await supabase
        .from("products")
        .insert({ name: name.trim(), branch_id: branchId } as any);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["products"] }),
  });
};

export const useDeleteProduct = () => {
  const qc = useQueryClient();
  const logActivity = useCreateActivityLog();
  return useMutation({
    mutationFn: async (id: string) => {
      const { data: prod } = await supabase
        .from("products")
        .select("name")
        .eq("id", id)
        .maybeSingle();
      const name = prod?.name ?? id;
      const { error } = await supabase
        .from("products")
        .update({ active: false } as any)
        .eq("id", id);
      if (error) throw error;
      logActivity.mutate({
        action: "product_deleted",
        description: `Product "${name}" deleted`,
        product_id: id,
      });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["products"] }),
  });
};
