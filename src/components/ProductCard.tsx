...
export const ProductCard = ({ product, entry, standalone = false }: { product: Product; entry?: ProductionEntry; standalone?: boolean }) => {
    ...
    <Card className={cn(
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {product.code && (
              <span className="text-[10px] px-2 py-0.5 rounded bg-primary/10 text-primary font-mono">{product.code}</span>
            )}
            {product.is_sub && (
              <span className="text-[10px] px-2 py-0.5 rounded bg-secondary text-muted-foreground uppercase tracking-wider">Sub</span>
            )}
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 w-full">
        <div className="flex flex-col items-center justify-center h-[80px] rounded-xl bg-white text-gray-900 shadow-md dark:bg-gray-900 dark:text-white dark:shadow-none">
          ...
          <p className="text-[10px] mt-1 text-center leading-tight uppercase text-gray-600 dark:text-gray-400">TARGET</p>
        </div>
        <div className="flex flex-col items-center justify-center h-[80px] rounded-xl border-2 border-green-500 bg-white text-gray-900 shadow-md dark:bg-gray-900 dark:text-white dark:shadow-none">
          ...
          <p className="text-[10px] mt-1 text-center leading-tight uppercase text-gray-600 dark:text-gray-400">COMPLETED</p>
        </div>
        <div className="flex flex-col items-center justify-center h-[80px] rounded-xl bg-white text-gray-900 shadow-md dark:bg-gray-900 dark:text-white dark:shadow-none">
          <p className="text-xl font-bold">{fmtNum(mp)}</p>
          <p className="text-[10px] mt-1 text-center leading-tight uppercase text-gray-600 dark:text-gray-400">MANPOWER</p>
        </div>
        <div className="flex flex-col items-center justify-center h-[80px] rounded-xl bg-white text-gray-900 shadow-md dark:bg-gray-900 dark:text-white dark:shadow-none">
          <p className="text-xl font-bold">{mp > 0 ? perWorker.toString() : "—"}</p>
          <p className="text-[10px] mt-1 text-center leading-tight uppercase text-gray-600 dark:text-gray-400">PER WORKER</p>
        </div>
      </div>
