...
export const Logo = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  ...
  return (
    <Link to="/" className="flex items-center gap-2.5 group">
      <div className={`grid place-items-center rounded-xl bg-gradient-primary p-2 shadow-glow-primary group-hover:scale-105 transition-smooth`}>
      ...
      <div className="flex flex-col leading-none">
        ...
        {size !== "sm" && <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Production Dashboard</span>}
      </div>
    </Link>
  );
};
